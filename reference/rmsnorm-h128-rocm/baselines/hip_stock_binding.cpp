// PyBind11 wrapper for hip_stock.hip — exposes the launcher to Python.
...
void launch(torch::Tensor x, torch::Tensor w, torch::Tensor y, double eps) {
    ...
    TORCH_CHECK(w.dim() == 1 && w.size(0) == x.size(1), "w must be [hidden] matching x.size(1)");
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("launch", &launch, "Launch hip_stock rmsnorm kernel (h=128 fast path or dynamic)");
}
